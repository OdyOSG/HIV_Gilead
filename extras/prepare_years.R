library(tidyverse)
dates <- 2016:2023
path <- tidyversepath <- "cohorts/1_target"
...
pwalk(grid, function(op = ..1, dt = ..2, ch = ..3) {
  ...
  cohortExpr$PrimaryCriteria$ObservationWindow$PriorDays <- 0
...
grid <- tidyr::expand_grid(
  op = c(182, 365, 728),
  ch = list.files(path, pattern = "Cohort_5")
)
pwalk(grid, function(op = ..1, dt = ..2, ch = ..3) {
  nm <- gsub(".json", "", ch)
  .f <- file.path(path, ch)
